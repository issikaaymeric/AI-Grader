...
export default function Dashboard() {
  ...
  // Native file dialog (Electron only)
  const openNativeDialog = async () => {
    if (!window.electronAPI) return;
    const filePath = await window.electronAPI.openFileDialog();
    if (filePath) {
      // Convert native path to a File-like object via fetch
      try {
        ...
        // fallback: just store path info
        setFile({ name: filePath.split(/[\\/]/).pop(), size: 0, _nativePath: filePath });
      }
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6">
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-lg p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Electron native dialog alternative */}
